      ...
      >
        ...
        <div className="map-overlay absolute top-4 left-4 p-4 pr-6 hidden sm:flex flex-col gap-1 z-20">
          <span className="text-[10px] font-sans font-bold text-brand-text-muted uppercase tracking-wider">
            {selectedCity ? `当前城市：${CITIES.find(c => c.name.toLowerCase() === selectedCity.toLowerCase())?.displayName || selectedCity}` : "最新打卡"}
          </span>
          {selectedCity ? (
            <div className="flex items-center gap-1.5 text-sm font-semibold text-brand-primary">
              <MapPin className="w-4 h-4" />
              <span>{CITIES.find(c => c.name.toLowerCase() === selectedCity.toLowerCase())?.displayName || selectedCity} • {filteredSips.length} 次饮记</span>
              <button 
            ...
            <div className="flex items-center gap-1.5 text-sm font-semibold text-brand-text">
              ...
              <span>{CITIES.find(c => c.name.toLowerCase() === latestSip.cityName.toLowerCase())?.displayName || latestSip.cityName} • {latestSip.drinkName}</span>
            </div>
          ) : (
        ...
        {CITIES.map((city) => {
          ...
          const citySips = sips.filter((s) => s.cityName.toLowerCase() === city.name.toLowerCase());
        ...
        {selectedCity && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20">
            ...
            >
              仅显示：{CITIES.find(c => c.name.toLowerCase() === selectedCity.toLowerCase())?.displayName || selectedCity} <MapPin className="w-3 h-3" /> (点击重置)
            </button>
          </div>
        )}
      </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {selectedCity && (
              <span className="bg-brand-primary/10 text-brand-primary text-xs px-2.5 py-0.5 rounded-full font-bold">
                {CITIES.find(c => c.name.toLowerCase() === selectedCity.toLowerCase())?.displayName || selectedCity}
              </span>
            )}
          </div>
          <button 
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-sans font-medium text-brand-text-muted flex items-center gap-0.5 truncate max-w-[65%]">
                      ...
                      <span className="truncate">
                        {CITIES.find(c => c.name.toLowerCase() === sip.cityName.toLowerCase())?.displayName || sip.cityName}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-1.5 mb-3">
                ...
                <div className="flex flex-wrap items-center gap-y-1 gap-x-3 text-xs text-brand-text-muted font-sans font-semibold">
                  <span className="flex items-center gap-1 text-brand-primary">
                    ...
                    {activeDetailSip.shopName} ({CITIES.find(c => c.name.toLowerCase() === activeDetailSip.cityName.toLowerCase())?.displayName || activeDetailSip.cityName}
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setActiveDetailSip(null)}
                  className="flex-1 py-2.5 bg-brand-surface hover:bg-brand-surface-low border border-brand-surface/30 text-brand-text-muted font-sans text-xs font-bold rounded-full transition-colors cursor-center text-center"
                >
                  合上照片
                </button>
                <button
