...
export default function DiscoverRankings({ onQuickCheckIn, sips = [] }: DiscoverRankingsProps) {
  ...
  const handleQuickLog = (item: { drinkName: string; shopName: string; category: DrinkCategory; imageUrl: string }) => {
    onQuickCheckIn({
  ...
  const getCitySpecials = (city: string) => {
    ...
    return rankings.slice(0, 2).map((item, idx) => ({
      ...
      description: `精选本地广受赞誉的“${item.drinkName}”，带有本地咖啡师精心调配的独特魅力风味，非常值得品鉴。`,
  ...
  const getPersonalBadgeInfo = (cat: DrinkCategory) => {
    switch (cat) {
      ...
      case DrinkCategory.PourOver:
        return { badge: "🫖 精品手冲极客", desc: "您是绝对的品质玩家，追求单品咖啡豆最干净本真的花香果酸，手冲仪式感满分。" };
      default:
        return { badge: "🌟 寻味探索大师", desc: "您拥有一条极富探险精神的多元化味蕾，每一种精彩的特色饮品都想品味一番。" };
    }
  };
